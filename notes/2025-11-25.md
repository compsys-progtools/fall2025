...
```{tip}
You should do *either* the `in groups` (if in class) or the `on your own` sections below for the sub-sub sections. (smaller, gray in the outline to the right ) For the section intros you need to do those no matter how you work (the main sections to the right).
```
...
```{include} ../_prepare/2025-12-02.md
```
